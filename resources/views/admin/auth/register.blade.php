...
<head>
    ...
    @php $authSettings = \App\Models\SiteSetting::current(); $authFavicon = $authSettings->favicon ?: 'images/bakeon-favicon.svg'; @endphp
    <title>{{ $authSettings->store_name ?? 'Bakeon' }} Admin | Register</title>
    <link rel="icon" href="{{ asset($authFavicon) }}?v=202609080435">
    <link rel="shortcut icon" href="{{ asset($authFavicon) }}?v=202609080435">
    <!-- Bootstrap 5 CSS -->
    ...
    <style>
        .auth-brand-logo{display:block;max-width:210px;max-height:76px;margin:0 auto 14px;object-fit:contain}
        .auth-brand-fallback{display:block;color:#ff1730;font-family:Georgia,'Times New Roman',serif;font-size:3rem;font-style:italic;font-weight:700;line-height:1;margin:4px auto 16px}
        :root {
    ...
    <div class="row justify-content-center">
        <div class="col-md-6 col-lg-5">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="text-center mb-4">
                        @if($authSettings->logo)
                            <img class="auth-brand-logo" src="{{ asset($authSettings->logo) }}" alt="{{ $authSettings->store_name ?? 'Bakeon' }}">
                        @else
                            <span class="auth-brand-fallback">bakeon</span>
                        @endif
            ...
            <p class="text-center text-muted small mt-4">
                &copy; {{ date('Y') }} {{ $authSettings->store_name ?? 'Bakeon' }} Administration. All rights reserved.
            </p>
        </div>
    </div>
</div>
